...
const userMainHeads = [
    ...
    {name:"accountNumber", dataStore:"value",default:null, isDefault:false},
    {name:"initialBalance", dataStore:"value",default:null, isDefault:false},
]
...
if(customersContent){
    customersContent.addEventListener('submit',function(e){
     ...
     if(head.isDefault) customers[head.name]=customersData.length==0?5000:customersData[customersData.length-1][head.name]+1;
...
function drawElements(){
    ...
    if(customersData.length==0){
        const tr = createMyOwnElement('tr',contentIndex, "alert alert-danger text-center")
        createMyOwnElement('td', tr,"", "No Customers Yet", [{attName:"colspan", attrVal:6}] )
    }
    else{
      customersData.forEach((customer, i)=>{
      const tr = createMyOwnElement('tr',contentIndex)
      userMainHeads.forEach( head=> createMyOwnElement('td', tr,"",customer[head.name]) )
      const td = createMyOwnElement('td',tr)
      const addBtn = createMyOwnElement('button', td, "btn btn-danger mx-3", "addPalance")
      addBtn.addEventListener('click', ()=> addPalance(customersData, customer.id))
      const withdrawBtn = createMyOwnElement('button', td, "btn btn-warning mx-3", "Withdraw")
      withdrawBtn.addEventListener('click', (e)=> withdraw(customersData, customer.id))
      const showBtn = createMyOwnElement('button', td, "btn btn-primary mx-3", "ShowCustomer")
...
if(addPalace){
    addPalace.addEventListener('submit',function(e){
        ...
        customersData[objIndex].transactions.push({type:"add",val:this.elements.amount.value}) ;
        setDataToStorage(customersData);
...
if(withdrawPalance){
    withdrawPalance.addEventListener('submit',function(e){
        ...
        //Find index of specific object using findIndex method.    
        objIndex = customersData.findIndex((obj => obj.id == id));
        customersData[objIndex].transactions.push({type:"withdraw",val:this.elements.amount.value}) ;
        setDataToStorage(customersData);
       // let customer=customersData.find(customer=>customer.id=id);   
        //customer.transactions.push({type:"add",val:this.elements.amount.value});
        console.log(customersData);
